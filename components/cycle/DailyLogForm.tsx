...
export function DailyLogForm({
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 flex flex-col gap-5">
      ...
      <div
        className={
          afficherDetailsEtendus ? 'flex flex-col gap-5' : 'grid grid-cols-1 gap-4 sm:grid-cols-2'
        }
      >
        {/* Énergie 1-5 */}
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from({ length: ENERGY_MAX }, (_, i) => i + ENERGY_MIN).map((val) => (
              <button
                ...
                ${
                  energie === val
                    ? 'bg-amber-600 text-white scale-110 dark:bg-amber-600'
      ...
      <Button
        onClick={sauvegarder}
        disabled={chargement}
        className="w-full bg-amber-600 text-white hover:bg-amber-700 dark:bg-amber-600 dark:hover:bg-amber-700 sm:w-auto sm:self-end"
      >
