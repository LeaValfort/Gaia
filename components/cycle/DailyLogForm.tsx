...
import { EXTENDED_LOG_INITIAL, extendedLogFromExisting } from '@/lib/data/journalOptions'
...
export function DailyLogForm({ date, phase, jourDuCycle, logInitial }: DailyLogFormProps) {
  ...
  const [extended, setExtended] = useState<ExtendedLogData>(
    logInitial ? extendedLogFromExisting(logInitial) : EXTENDED_LOG_INITIAL
  )
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-5 flex flex-col gap-5">
      ...
      {/* Douleur 0-10 */}
      <div>
        <p className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-2">Douleur</p>
        <div className="flex gap-1 flex-wrap">
          {Array.from({ length: PAIN_MAX + 1 }, (_, i) => i + PAIN_MIN).map((val) => (
            <button key={val} type="button" onClick={() => setDouleur(val)}
      ...
      {/* Humeur (texte libre court) */}
      <div>
        <p className="text-xs font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wide mb-2">Humeur</p>
        <input
          type="text"
          value={humeur}
          onChange={(e) => setHumeur(e.target.value)}
          placeholder="Comment tu te sens ? (ex: motivée, fatiguée, zen...)"
          className="w-full px-3 py-2 text-sm rounded-lg border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-neutral-900 dark:text-neutral-50 placeholder:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-violet-400"
        />
      </div>
